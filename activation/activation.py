...
def parse_args():
    p = argparse.ArgumentParser(
        description="Полный пайплайн: снять активации FFN-нейронов на двух языках, "
                    "посчитать вероятности/энтропию (LAPE-style) и выделить языко-специфичные нейроны."
    )
    ...
    # дополнительные управляемые параметры
    p.add_argument("--model", type=str, default="Tweeties/tweety-tatar-base-7b-2024-v1",
                   help="Имя/путь HF-модели (Mistral/LLaMA/BLOOM).")
    p.add_argument("--batch_size", type=int, default=4, help="Размер батча предложений.")
    p.add_argument("--max_length", type=int, default=512, help="Максимальная длина в токенах.")
    p.add_argument("--limit", type=int, default=0, help="Ограничение на число строк (0 = без).")
    p.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu",
                   help="cuda или cpu.")
    p.add_argument("--filter_rate", type=float, default=0.95,
                   help="Перцентиль по p(активации) для фильтра ‘существенных’ нейронов.")
    p.add_argument("--top_rate", type=float, default=0.01,
                   help="Доля (например, 0.01=1%) нейронов с минимальной энтропией для отбора.")
    p.add_argument("--activation_bar_ratio", type=float, default=0.95,
                   help="Перцентиль по p(активации) как планка для приписывания языка нейрону.")
    p.add_argument("--save_json", type=str, default="neuron_stats_two_lang.json",
                   help="Куда сохранить сводную статистику (JSON).")
    p.add_argument("--save_mask_pth", type=str, default="two_lang.neuron.pth",
                   help="Куда сохранить маски языко-специфичных нейронов (torch.save).")
    return p.parse_args()


# ====== Вспомогательные функции ======

def load_texts(csv_path: str, c1: str, c2: str, limit: int) -> Tuple[List[str], List[str]]:
    print(f"[1/8] Загрузка текстов из {csv_path}...")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Не найден CSV: {csv_path}")
    df = pd.read_csv(csv_path)
    if c1 not in df.columns or c2 not in df.columns:
        raise ValueError(f"В CSV должны быть колонки '{c1}' и '{c2}'. Найдены: {list(df.columns)}")
    t1 = [str(x).strip() for x in df[c1].tolist() if isinstance(x, str) and str(x).strip()]
    t2 = [str(x).strip() for x in df[c2].tolist() if isinstance(x, str) and str(x).strip()]
    n = min(len(t1), len(t2))
    t1, t2 = t1[:n], t2[:n]
...
def get_layers_and_intermediate(model) -> Tuple[List[Any], int]:
    ...
    if hasattr(model, "model") and hasattr(model.model, "layers"):
        ...
        raise RuntimeError("Не удалось найти список слоёв модели (ожидались .model.layers или .transformer.h).")
...
def run_pass_and_count(
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if batch_idx % 10 == 0 or batch_idx == num_batches:
            print(f"    Обработано батчей: {batch_idx}/{num_batches} ({total_tokens} токенов)")
    return total_tokens


def make_patchers(layers: List[Any], over_zero_ref: Dict[str, torch.Tensor]) -> List[Any]:
    """
...
def main():
    ...
    # 1) Данные
    lang1_texts, lang2_texts = load_texts(args.csv, args.lang1_col, args.lang2_col, args.limit)

    # 2) Модель и токенайзер
    print(f"[2/8] Загрузка токенайзера из {args.model}...")
    tokenizer = AutoTokenizer.from_pretrained(args.model, use_fast=True)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    print(f"[3/8] Загрузка модели {args.model}...")
    ...
    print(f"  Модель загружена: {num_layers} слоёв, {intermediate_size} нейронов/слой, всего {total_neurons} нейронов")

    # 3) Счётчики активаций для обоих языков
    over_zero_1 = torch.zeros((num_layers, intermediate_size), dtype=torch.int64, device=device)
    over_zero_2 = torch.zeros((num_layers, intermediate_size), dtype=torch.int64, device=device)

    # 4) Патчим forward, прогоняем язык 1
    print(f"[4/8] Прогон текстов lang1 ({args.lang1_col}) через модель...")
    ...
    tokens_1 = run_pass_and_count(
        lang1_texts, tokenizer, model, device, args.batch_size, args.max_length, over_zero_1
    )
    print(f"  Lang1 завершён: {tokens_1} токенов обработано")

    # 5) Патч остаётся, меняем счётчик и прогоняем язык 2
    print(f"[5/8] Прогон текстов lang2 ({args.lang2_col}) через модель...")
    over_zero_ref["tensor"] = over_zero_2
    tokens_2 = run_pass_and_count(
        lang2_texts, tokenizer, model, device, args.batch_size, args.max_length, over_zero_2
    )
    ...
    k_filter = max(1, int(round(len(flat_probs) * args.filter_rate)))
    ...
    print(f"[7/8] Отбор языко-специфичных нейронов (top {args.top_rate*100:.1f}% по энтропии)...")
    flat_entropy = entropy_filtered.view(-1)
    k_top = max(1, int(round(len(flat_entropy) * args.top_rate)))
    ...
    k_bar = max(1, int(round(len(flat_probs) * args.activation_bar_ratio)))
    ...
    result: Dict[str, Any] = {
        "model": args.model,
        "csv": args.csv,
        "columns": {"lang1": args.lang1_col, "lang2": args.lang2_col},
        "device": str(device),
    ...
    torch.save([by_lang[0], by_lang[1]], args.save_mask_pth)

    print("\n=== DONE ===")
    print(f"Модель: {args.model}")
    print(f"Слои: {num_layers} | intermediate_size: {intermediate_size} | всего нейронов: {total_neронов if False else total_neurons}")
    print(f"Токенов обработано: lang1={int(tokens_1)}, lang2={int(tokens_2)}")
    print(f"Активные (≥1 раз): lang1={int(active1_any.sum().item())}, lang2={int(active2_any.sum().item())}, оба={int(both.sum().item())}")
    print(f"Языко-специфичные (по порогу {args.activation_bar_ratio:.2f} и топ-{args.top_rate*100:.1f}% низк. энтропии) — "
    ...
    print(f"Masks .pth: {args.save_mask_pth}")
