...
def parse_args():
    ...
    p.add_argument(
        "--lang1_col", type=str, required=True, help="Имя колонки с первым языком."
    )
    p.add_argument(
        "--lang2_col", type=str, required=True, help="Имя колонки со вторым языком."
    )
    ...
    p.add_argument(
        "--model",
        type=str,
        default="Tweeties/tweety-tatar-base-7b-2024-v1",
        help="Имя/путь HF-модели (Mistral/LLaMA/BLOOM).",
    )
    p.add_argument(
        "--batch_size", type=int, default=4, help="Размер батча предложений."
    )
    p.add_argument(
        "--max_length", type=int, default=512, help="Максимальная длина в токенах."
    )
    p.add_argument(
        "--limit", type=int, default=0, help="Ограничение на число строк (0 = без)."
    )
    p.add_argument(
        "--device",
        type=str,
    ...
    p.add_argument(
        "--save_json",
        type=str,
        default="neuron_stats_two_lang.json",
        help="Куда сохранить сводную статистику (JSON).",
    )
    p.add_argument(
        "--save_mask_pth",
        type=str,
        default="two_lang.neuron.pth",
...
def main():
    ...
    lang1_texts, lang2_texts = load_texts(
        args.csv, args.lang1_col, args.lang2_col, args.limit
    )
    ...
    print(
        f"[7/8] Отбор языко-специфичных нейронов (top {args.top_rate*100:.1f}% по энтропии)..."
    )
    ...
    selected_probs = probs[
        row_index, col_index
    ]  # (k_top, 2) — вероятности по двум языкам
    ...
    print(
        f"Слои: {num_layers} | intermediate_size: {intermediate_size} | всего нейронов: {total_neронов if False else total_neurons}"
    )
    ...
    print(
        f"Языко-специфичные (по порогу {args.activation_bar_ratio:.2f} и топ-{args.top_rate*100:.1f}% низк. энтропии) — "
